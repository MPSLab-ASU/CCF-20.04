...
typedef struct thread_args{
  char* string;
  size_t pos, limit, shift;
  char* here;
} thread_args;

void thread_search(thread_args* args){
  char* string = args->string;
  register size_t pos = args->pos;
  size_t limit = args->limit, shift = args->shift;
  char* here = args->here;
  //printf("Thread search running\n");
  while (pos < limit)
    {
      while( pos < limit &&
...
char *strsearch(const char *string)
{
      ...
      thread_args args = {string, pos, limit, shift, here};
      pthread_t loop_thread;
      pthread_create(&loop_thread, NULL, (void*) thread_search, &args);
      pthread_join(loop_thread, NULL);
      //printf("Thread search joined\n");
